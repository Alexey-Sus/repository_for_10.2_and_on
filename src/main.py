...
def main():
    ...
    # ввод пользователем статуса операции и проверка этого ввода
    while True:
        status_trans: str = input('Введите статус, по которому необходимо выполнить фильтрацию.\n'
    ...
    # ввод пользователем выбора сортировки по дате и проверка этого ввода
    while True:
        sorted_date: str = input("It's not over yet. Нужно сортировать по дате? да/нет: \n ")
    ...
    if srt_d:
        while True:
            ...
            if sorted_reverse.lower() in ['по возрастанию', 'по убыванию']:
                ...
                print('Вы ввели что-то не то. Вводите только "по возрастанию" или "по убыванию": ')
        print(f'Вы ввели статус сортировки по возрастанию или убыванию: "{sorted_reverse.lower()}", {srt_rev}')
    else:
        srt_rev = False
        pass
    # ввод пользователем рублевых или non-рублевых транзакций и проверка этого вывода
    while True:
        rub_or_all: str = input("Stay here for a bit more. Выводить только RUB-транз или все? да/нет: \n")
    ...
    print(f'Вы ввели статус транзакций: {'только рублевые' if RUB_trans else '"по всем валютам"'}')

    # ввод пользователем выбора фильтрации тр-ий по слову в описании и (если да) ввод этого
    # слова и проверка этого ввода
    while True:
        word_in_descr: str = input("Your nightmare is still here. Wanna some word to filter trxs out? да/нет: \n")

        if word_in_descr.lower() == 'да':
            wrd_descr: bool = True
            wrd_to_use: str = input('So please input here your lovely word(s) to filter trxs out:\n')
            break
    ...
    def output_json(result: list, tr_st: str, srtd_date: bool, srtd_rev: bool, RUB_tr, wrd_dsc, wrduse) -> list:
        ...
        if srtd_date:
            list_sorted_date: list = sorted(list_output, key=lambda i: i.get('date'), reverse=srtd_rev)
        ...
        if RUB_tr:
            list_for_rub: list = []
            for х in list_output:
                if х['operationAmount']['currency']['code'] == 'RUB':
                    list_for_rub.append(х)
    ...
    def output_csv_excel(result: list, tr_st: str, srtd_date: bool, srtd_rev: bool,
                         RUB_tr: bool, wrd_dsc: bool, wrduse: str) -> list:
    ...
    # делаем вызов нужной функции, в зависимости от того, какой у нас первый выбор - 1, 2 или 3:
    if first_choice == '1':
        result = output_json(result_interm, status_trans, srt_d, srt_rev, RUB_trans, wrd_descr, wrd_to_use)
    elif first_choice in ['2', '3']:
        result = output_csv_excel(result_interm, status_trans, srt_d, srt_rev, RUB_trans, wrd_descr, wrd_to_use)

    print('Распечатываю итоговый список транзакций...')
    print(f'Всего банковских операций в выборке: {len(result)}')

    # дальше список и делаем его вывод так, чтобы он соответствовал условиям задачи (по три строки на каждую операцию,
    for i in result:
        print()
